...
function BucketItem({args,bucket}) {
	const { flag_change, bucket_update, bucket_complete, bucket_cancle} = args
	const [b_update, setB_Update] = useState({
		b_title: "",
		isEdit: false,
	})
	const onItemClick = (e) => {
		// currentTarget -> 직접 이벤트가 설정된 tag
	...
	const bucket_KeyDown = (e) => {
		if(e.keyCode === 13){
			 ...
			 */
			// console.log(`test : ${b_id} n ${b_title}`)
  ...
  return (
    <tr className={bucket.b_cancle ? "cancle" : ""}key={bucket.b_id} data-id={bucket.b_id} onClick={onItemClick}>
      <td className="b_flag">
      ...
      <td>
        <input type="checkbox" checked={bucket.b_cancle}/>
      </td>
    </tr>
  );
}
