...
function BBsWrite() {
  ...
  /**
   * useRouteMatch()
   * URL을 통해서 전달된 데이터들 queryString, pathVarriable
   * ?변수=값 /URL/값
   */
  const match = useRouteMatch();
  /**
   * /write/:id로 설정된 Route에서 id위치에 담긴 변수 값 가져오기
   */
  ...
  const onClickSave = () => {
    ...
    const saveBBS = {
      ...bbs,
      /**
       * 원래 날짜와 시간이 있으면 그 값을 사용하고
       * 없으면 뒤에 있는 moment로 새로 작성하라
       * 수정할 때 날짜와 시간이 바뀌지 않게 만들기 위함!
       *
       * bbs.b_date 의 값이 ""이 아니면
       * 	bbs.b_date를 b_date 칼럼에 저장하고
       * bbs.b_date 의 값이 "" dlaus
    ...
    firestore
      .collection("bbs")
      //   .add(saveBBS)
      .doc(docId) // (docId)를 넣어줘야 수정이 가능해진다
      .set(saveBBS)
      .then((result) => {
        console.log(result);
        history.push("/");
