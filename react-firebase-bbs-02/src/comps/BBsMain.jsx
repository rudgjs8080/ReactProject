...
function BBsMain() {
  ...
  const firebaseFetch = async () => {
     ...
     *
     * collection.where() 조건 지정이 가능하다
     *
     * 조건지정을 할 때 부등호(<,>)가 들어가는 조건을 사용할 경우는
     * 반드시 해당 칼럼을 먼저 정렬해줘야한다
     * collection.orderBy("b_date").where('b_date > "2021-09-01" ')
     *
     * orderBy() 두 개 이상의 칼럼을 지정할 경우
     * fireStore 콘솔에서 복합색인을 추가해 줘야 한다
     *
    ...
    /**
     * firestore로 부터 수신된 데이터 중에서 실제 BBS 데이터 객체만
     * 추출하여 bbsList 배열로 만들기
     */
