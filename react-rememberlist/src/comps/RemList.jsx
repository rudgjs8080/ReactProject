import React from 'react'
import { useState, useEffect, useCallback } from 'react';
import moment from 'moment'
import UUID from 'react-uuid'


const headArray = ['날짜','날짜', '시간', '기억할 일']


const rememberSampleData = {
	r_id: '0001',
	r_date: '2021-09-08',
	r_time: '10:33:12',
	r_remember: '나의 리멤버 리스트',
	r_comp: false // 완료표식
}

function RemList() {

	/**
	 * headArray 배열의 요소들을 text 라고 한 후
	 * <th>{text}</th>로 반복 생성 후 return
	 * 
	 * useCallback
	 * 함수를 선언하는 이유 : 반복적으로 사용하는 코드를 하나로 묶어서
	 * 		함수 이름을 통해서 한 개의 명령처럼 사용하기 위함
	 * 
	 * const f = () => {
	 * 		console.log('대한민국')
	 * }
	 * 
	 * 선언된 함수를 3번 호출하면
	 * f()
	 * f()
	 * f()
	 * => 
	 * 대한민국
	 * 대한민국
	 * 대한민국
	 * 
	 * 위의 예제는 함수를 1번 만들고
	 * 3번 활용한 것
	 * 
	 * 일반적인 함수는 사용하기 전에 만들고, 사용을 계속하고...하는 로직의 연속인데
	 * 
	 * react는 한 번 rendering된 화면은 특별한 값(상태)의 변화가 발생하면
	 * 그 부분만 다시 rendering 하는 동작이 발생한다
	 * 
	 * 어떤 화면(컴포넌트)이 하나 생성되었는데
	 * 그 화면을 다시 열고, 닫는 동작이 반복될때
	 * 내부의 코드는 변함이 없다(데이터가 변하지 않기 때문)
	 * 이런 과정에서 선언된 함수는 화면이 다시 열릴때 마다 반복적으로
	 * 다시 선언되고 rendering되게 된다 (불필요한 rendering)
	 * 코드는 변함이 없고 내부에 화면을 그렸던 코드는
	 * 이전에 생성된 코드가 있는데
	 * 함수만 계속해서 반복적으로 선언, 생성이 되니 불편하다
	 * 
	 * 그래서 함수를 usecallback( 함수() )로 감싼다
	 * 함수()를 usecallback이 보관하고 있다가
	 * 다음에 함수()가 필요할 때 나에게 다시 달라?
	 * 
	 * 함수() 가 이미 생성된 적이 있으면
	 * 재활용을 하고 없을 때만 다시 rendering해서 사용한다는 의미?
	 */
	const rem_header = useCallback(() => {
		return headArray.map(text => {
			return(
				<th key={UUID()}>{text}</th>
			)
		})
	}, [])


	/**
	 * rememberList를 담을 배열을 "상태"로 선언하기
	 */
	const [rememberList, setRememberList] = useState([rememberSampleData])
	const saveStorage = () => {
		// console.log('effect')
	/**
	 * rememberList 객체 배열에 담긴 데이터를
	 * JSON String 문자열로 변환하여 JSON.stringify()
	 * localStorage에 rememberList 라는 이름으로 저장하라
	 */
		if(rememberList.length > 0){
			/**
			 * 객체 배열 sort하기
			 * 배열.sort(compareFunction(p,n))
			 * sort를 만나면 forEach가 작동된다
			 * 
			 * compareFunction의 return 값에 따라서 배열의 위치가 교환된다
			 * return 1 : next 가 앞으로, pre가 뒤로 이동
			 * return -1 : pre가 앞으로
			 * return 0 : 그대로 있어라
			 * 
			 * map(), filter() 는 결과를 return 하여 다른 배열을 생성한다
			 * sort()는 자기 자신을 변경한다
			 */
			rememberList.sort((pre,next) => {
				return pre.r_comp
				? 1 
				: pre.r_date > next.r_date && pre.r_time > next.r_time 
				? 1
				: -1
			})


			localStorage.rememberList = JSON.stringify(rememberList)
		}
		
	}

	// useEffect(함수, 상태대상)
	// 화면에 rendering이 발생할 때 실행되는 public event 연결
	// 일부러 호출하거나, 실행하지 않아도
	// 어떤 조건이 발생하면 자동으로 

	/**
	 * useEffect가 실행(호출할) 함수를 선언하였다
	 * 이 함수는 화면이 rendering될 때 한번만 호출될 함수
	 * 하지만 react에 의해서 현재 화면이 보여지는 상태가 되면
	 * 이 함수를 계속해서 다시 생성한다
	 * 현재의 컴퓨터 성능으로는 큰 문제가 없지만
	 * 이러한 과정이 계속해서 반복된담년 메모리 등에 문제를 일으 킬 수 있다
	 * 
	 * react 16 에서는 이러한 함수를 useCallback()으로 감싸도록 권장하고 있다
	 * useCallback()으로 감싸진 함수는 
	 * 이전에 한 번이라도 만들어진 경우는 그대로 재 사용하고
	 * 없는 경우에만 함수를 생성한다
	 */
	
	const fetchCallback = useCallback(() => {
		const remString = localStorage.rememberList
		if(remString){
			console.log('Fetch remeberList')
			const remJSON = JSON.parse(remString)
			setRememberList(remJSON)
			console.log('json',remJSON)
		}
	}, [])
	useEffect(fetchCallback, [fetchCallback]) 
	useEffect(saveStorage, [rememberList]) // rememberList에 변화가 생기면 saveStorage 함수 실행


	/**
	 * list 중 한 요소를 더블클릭하면
	 * 선택된 요소의 UUID값을 추출하여
	 * 1. 해당요소를 삭제하기
	 * 2. 해당 요소의 r_comp 값을 완료된 것으로 표시하기
	 */
	const trOnClick = (e) => {
		const td = e.target
		if(td.tagName === "TD"){
			const uuid = td.closest("TR").dataset.uuid
			// alert(uuid)

			// // 리스트에서 선택된 요소의 UUID값이 담긴 것만 빼고(filtering)
			// // 새로운 복제 _list를 만들기
			// const _list = rememberList.filter(remember => {
			// 	return remember.r_id !== uuid // uuid와 다른것만 return
			// })
			// // filetering된 list를 rememberList로 대체하기
			// setRememberList([..._list])

			const _list = rememberList.map(remember => {
				if(remember.r_id === uuid) {
					return {...remember, r_comp: !remember.r_comp }
				} return remember
			})
			setRememberList([..._list])
		}
	}
	const list_body = rememberList.map((remember) => {
		return (
			<tr
				key={remember.r_id}
				data-uuid={remember.r_id}
				className = {remember.r_comp ? "comp" : ""}
				onDoubleClick={trOnClick}
			>
				<td>{remember.r_id}</td>
				<td>{remember.r_date}</td>
				<td>{remember.r_time}</td>
				<td>{remember.r_remember}</td>
			</tr>
		)
	})

	/**
	 * input box 입력을 하는 과정에서 Enter를 누르면
	 * 
	 * 데이터를 어딘가에 추가하기(rememberlist 상태에)
	 * 
	 */

	const onKeyDown = (e) => {
		// 키보드로 입력을 하는 도중 Enter를 누르면
		if(e.keyCode === 13) {
			const { value } = e.target
			// alert(value)
			// input box에 입력된 문자열을 rememberlist에 담기위해서 객체로 생성
			const remember = {
				r_id : UUID(),
				r_date : moment().format("YYYY[-]MM[-]DD"),
				r_time : moment().format("HH:mm:ss"),
				r_remember: value,
				r_comp : false
			}
			/**
			 * const ar = [1,2,3,4,5]
			 * const arCopy =  [...ar] 새로운 arCopy로 복제
			 * 
			 * const arCopyAdd = [...ar,9,10]
			 * arCopyAdd => [1,2,3,4,5,9,10]
			 * 
			 */
			// 생성된 remember 객체 데이터를 rememberList 상태에 추가하기
			// setRememberList(rememberList.concat(remember))
			// rememberList = ?? 이 아니고
			// 기존 rememberList를 새로운 배열로 대치
			setRememberList([...rememberList, remember]) // 전개 연산자
			e.target.value=""; // 입력박스에 있는 내용 지우기 

		}
	}
	return (
		<table class='rem_list'>
			<tr>{rem_header()}</tr>
			<tbody>
				{list_body}
			<tr>
				<td colSpan="2">기억할 일</td>
				<tr>
					<input 
					onKeyDown={onKeyDown}
					name="r_remember" 
					placeholder="기억 할 일"></input>
				</tr>
			</tr>
			</tbody>
		</table>
	)
}

export default RemList
